    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-4">
                <div>
                    <label className="text-base font-medium">템플릿 이름</label>
                    <input type="text" value={template.name} onChange={e => setTemplate(t => ({...t, name: e.target.value}))} className="w-full mt-1 border-slate-300 rounded-md" />
                </div>
                <div>
                    <label className="text-base font-medium">보고서 목차 (섹션)</label>
                    {template.sections.map((section, index) => (
                         <input key={index} type="text" value={section} onChange={e => handleSectionChange(index, e.target.value)} className="w-full mt-1 border-slate-300 rounded-md" />
                    ))}
                </div>
                 <div>
                    <label className="text-base font-medium">AI 스타일 가이드</label>
                    <select value={template.aiStyleGuide} onChange={e => setTemplate(t => ({...t, aiStyleGuide: e.target.value}))} className="w-full mt-1 border-slate-300 rounded-md">
                        <option>간결/전문가체</option>
                        <option>상세/설명조</option>
                    </select>
                </div>
                <div>
                    <label className="text-base font-medium">결재란</label>
                    <input type="text" value={template.approvalBox} onChange={e => setTemplate(t => ({...t, approvalBox: e.target.value}))} className="w-full mt-1 border-slate-300 rounded-md" placeholder="예: 작성-검토-승인" />
                </div>
            </div>
            <div className="md:col-span-1 bg-slate-50 p-4 rounded-lg">
                <h4 className="font-semibold text-base">사용 가능 변수</h4>
                <p className="text-sm text-slate-500 mb-2">본문에 삽입 시 자동으로 데이터가 채워집니다.</p>
                <div className="space-y-1">
                    {placeholders.map(p => <code key={p} className="text-sm block bg-slate-200 text-slate-700 p-1 rounded">{p}</code>)}
                </div>
            </div>
        </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {templates.map((template, index) => (
                <div key={index} className="space-y-4 border p-4 rounded-lg">
                    <h4 className="font-bold text-lg text-slate-800">{template.channel.toUpperCase()} / {template.level}</h4>
                     <div>
                        <label className="text-base font-medium">제목</label>
                        <input type="text" value={template.subject} onChange={e => handleTemplateChange(index, 'subject', e.target.value)} className="w-full mt-1 border-slate-300 rounded-md" />
                    </div>
                     <div>
                        <label className="text-base font-medium">본문</label>
                        <textarea rows={5} value={template.body} onChange={e => handleTemplateChange(index, 'body', e.target.value)} className="w-full mt-1 border-slate-300 rounded-md" />
                    </div>
                    <div className="bg-slate-50 p-2 rounded-md">
                        <p className="text-sm text-slate-500 mb-1">사용 가능 변수: {placeholders.join(', ')}</p>
                        <p className="text-sm text-slate-600 font-mono border-t pt-2 mt-2"><b>미리보기:</b> {template.body.replace('{{asset_name}}', '한강교 A3 교각').replace('{{current_value}}', '9.1').replace('{{threshold_value}}', '8.0')}</p>
                    </div>
                </div>
            ))}
        </div>
        ...
        <div className="space-y-8">
            <div>
                <div className="flex items-center gap-2">
                    <h2 className="text-4xl font-bold text-slate-900">템플릿 관리</h2>
                    <div className="relative group">
                        ...
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-80 p-3 bg-slate-800 text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                            <h4 className="font-bold mb-1 border-b pb-1">템플릿 관리 도움말</h4>
                            <p className="mt-2">
                            ...
                            <ul className="mt-2 space-y-1 list-disc list-inside">
                                {/* FIX: Replaced backticks and bare curly braces with a `<code>` tag and a JSX string literal to prevent parsing errors. */}
                ...
                <p className="mt-1 text-lg text-slate-600">AI 리포트 및 자동 알림의 표준 양식을 설정합니다.</p>
            </div>
            ...
            <Card className="p-0">
                ...
                <div className="px-6 py-4 bg-slate-50 border-t flex justify-end">
                    <button className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md text-base">변경사항 저장</button>
                </div>
            </Card>
        </div>
    );
